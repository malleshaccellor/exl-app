...
/** Parse a content string that may have a styled <div> wrapper (from blockToStyledHtml).
 *  Returns both the leaf children AND any block-level styles (align, indent, fontSize). */
const parseContentHtml = (str: string): { children: any[]; styles: Record<string, any> } => {
  if (!str) return { children: [{ text: "" }], styles: {} };

  // Detect styled wrapper: <div style="...">content</div>
  const styledMatch = str.match(/^<div style="([^"]*)">([\s\S]*)<\/div>$/);
  if (styledMatch) {
    const styleStr = styledMatch[1];
    const innerContent = styledMatch[2];
    const styles: Record<string, any> = {};

    const alignMatch = styleStr.match(/text-align:\s*(\w+)/);
    if (alignMatch) styles.align = alignMatch[1];

    const indentMatch = styleStr.match(/padding-left:\s*(\d+)px/);
    if (indentMatch) styles.indent = Math.round(parseInt(indentMatch[1], 10) / 24);

    const fontMatch = styleStr.match(/font-size:\s*(\d+)px/);
    if (fontMatch) styles.fontSize = fontMatch[1];

    return { children: parseInlineHtml(innerContent), styles };
  }

  return { children: parseInlineHtml(str), styles: {} };
};
...
const createBrdBulletedList = (items: string[]): any => {
  return {
    type: "bulleted-list" as const,
    children: items.map((item) => {
      const { children, styles } = parseContentHtml(String(item));
      return {
        type: "list-item" as const,
        ...styles,
        children,
  ...
  return Object.entries(obj).flatMap(([key, value]) => {
    if (
      ...
      const subNodes = Object.entries(value).flatMap(([subKey, subValue]) => {
        const { children, styles } = parseContentHtml(String(subValue));
        return [
          {
