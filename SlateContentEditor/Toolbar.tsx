...
import type { AlignType, textListStyle, textStyle } from "./types";
...
const toggleFontSize = (editor: Editor, fontSize: string) => {
  ...
  const newFontSize = fontSize.trim() === "" ? undefined : fontSize;
...
const toggleAlign = (editor: Editor, align: AlignType) => {
  ...
  // 1. Unhang the range to avoid affecting multiple blocks accidentally
  const range = Editor.unhangRange(editor, selection);

  // 2. Apply the alignment to all matching blocks in the selection
  Transforms.setNodes(
    editor,
    { align },
    {
      at: range,
      match: (n) => SlateElement.isElement(n) && Editor.isBlock(editor, n),
      mode: "lowest", // Targets the most specific block (e.g., paragraph, not a list wrapper)
    },
  );
};
